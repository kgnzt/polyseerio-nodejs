...
/**
 * Delegates how the client is constructed.
 */
function delegate (...args) {
  if (args.length === 1 && lodash.isObject(args[0])) {
    const config = args[0],
          client = factory(config.token, {
            deduce: true
          });

    client.agent = new Agent(client);
...
delegate.ClientResourcePaths = ClientResourcePaths;
delegate.RequiredResources = RequiredResources;

Object.assign(delegate, Enum);
