...
function _factory (resource, request, options = {}) {
  ...
  const defintion = DEFINITION[resource],
        Resource = definesSingleton(DEFINITION[resource]) ? {} : createResource(resource);

  const functions = staticFactory(request, resource, defintion[STATIC], options),
        methods = instanceFactory(request, resource, defintion[METHOD], options);
