 ...
 * @return {function}
 */
function createHandlerIteratee (handlers, iteratee, ...args) {
  return function (value, item) {
    if (lodash.has(handlers, item)) {
      return iteratee(handlers, value, item, ...args);
    }
  
    return global.Promise.reject(new Error(`Handlers did not contain a type: ${item}.`));
  };
}
...
function _setup (handlers, ...args) {
  return createHandlerIteratee(handlers, (handlers, value, item, ...args) => {
