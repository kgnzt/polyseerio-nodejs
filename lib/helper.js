 ...
 * @return {object}
 */
function wrapRequest (request, pre, post) {
  pre  = pre  || lodash.identity;
  post = post || lodash.identity;

  const wrapper = options => request(pre(options)).then(post);
  ...
  ['del',
   ...
   'put'
    ...
    wrapper[method] = options => {
      return request[method](pre(options)).then(post);
    };
  });
