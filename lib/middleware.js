 ...
 * @return {Promise}
 */
function retryRequest (request, failed, payload = {}) {
...
function rejectedDueToMissingEnvironment (error) {
  ...
  return lodash.some(error.error.errors, err => {
    return (err.status === 404 && err.detail.indexOf('Could not find the environment') !== -1);
  });
}
...
function postReject (error, cid, request) {
  const originalPayload = JSON.parse(error.response.request.body), // becomes undefined (tick issue)
        isMissingEnvironment = rejectedDueToMissingEnvironment(error);
  ...
  if (isMissingEnvironment) {
    ...
    return upsertEnvironment(request, name).
      then(_ => {
        return retryRequest(request, failedRequest, originalPayload);
      });
  }
